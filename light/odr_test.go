...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	blockchain, _ := core.NewBlockChain(sdb, nil, params.TestChainWithAllForksConfig, ethash.NewFullFaker(), vm.Config{}, nil, nil)
	gchain, _ := core.GenerateChain(params.TestChainWithAllForksConfig, genesis, ethash.NewFaker(), sdb, 4, testChainGen)
	if _, err := blockchain.InsertChain(gchain); err != nil {
		t.Fatal(err)
	}

	odr := &testOdr{sdb: sdb, ldb: ldb, indexerConfig: TestClientIndexerConfig}
	lightchain, err := NewLightChain(odr, params.TestChainWithAllForksConfig, ethash.NewFullFaker(), nil)
	if err != nil {
		t.Fatal(err)
	}
